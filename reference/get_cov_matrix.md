...
`"om.<eta name>"` to distinguish them from the point estimates returned
by
[`get_prm()`](https://jprybylski.github.io/xpose.xtras/reference/get_prm.md).
`drop_fixed` has no effect here, since parameters without a standard
error are already excluded from the fit's covariance matrix rather than
represented as placeholder zeros.
