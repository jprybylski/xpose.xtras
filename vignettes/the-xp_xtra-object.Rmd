...
Note that worked even though `boxplot_fill` is not defined in the `theme_xp_xpose4()`, and that function is not masked by `xpose.xtras`. To update an existing theme object to one for the extension pack (instead of using the old theme in the `xp_theme` argument), simply use `updated_theme <- xp_xtra_theme(old_theme)`.
