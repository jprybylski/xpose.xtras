...
A powerful new object building upon the `xpose` framework is the `xpose_set`. Much like how `xpose_data` is in essence a list of information and data about the fitted model, `xpose_set` is a list of `xpose_data` (or `xp_xtra`) objects with information about how these models relate to one another.

Creating a set is easy. An important point to remember about usage is that each set item must have a label. Depending on how the set is created, the default is to use the name of the `xpose_data` object, but if an unnamed list is spliced or any objects share the same label, an error will occur. Note, however, that duplicate `xpose_data` objects can be used, as long as they have different labels. In the example below, three example models fitting the same dataset are made into a set, and alternative labels are used for one version of the set.
...
The `print` output hints at features that will be explored in this vignette. A few example sets are in the package that can be used to test some of the elements discussed here. One relatively complex one explores typical model-building steps for the common phenobarbital in neonates dataset, called `pheno_set`, diagrammed below.
...
The method above limits the comparison to the longest lineage in the provided set, starting at a base model if one is declared. The models included can be examined by probing the `xset_lineage` function.
...
Two models can be compared using an updated version of a `xpose4` function; this is referred to in some cases as a "shark plot", and it is called `dOFV.vs.id()` in `xpose4`. As such, it is called `shark_plot` or `dofv_vs_id` in `xpose.xtras`.
...
Information from `xpose_data` summary or parameter values (properties) can be "exposed", which means they become associated with the set item on the top level. To view these as a table, the function `reshape_set` can be used. Note the exposed properties are denoted by prefixed `..` in their column names.
