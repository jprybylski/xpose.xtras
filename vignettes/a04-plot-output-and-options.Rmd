...
apply_default_labs(p, caption = "Run @runno -- @nobs observations", overwrite = TRUE)
...
```{r default_labs_xpdb}
xpdb_draft <- xpdb_x %>%
  set_default_labs(caption = "Model @runno -- interim, subject to change")
...
list.files(out_dir, pattern = "dv_vs_ipred_annotated")
```
...
```{r ggsave_xp_cleanup, include = FALSE}
unlink(list.files(out_dir, pattern = "dv_vs_ipred|run001", full.names = TRUE))
```
...
dv_vs_pred(xpdb_ex_pk, quiet = TRUE) %>%
  apply_default_labs() %>%
  add_watermark()
```
